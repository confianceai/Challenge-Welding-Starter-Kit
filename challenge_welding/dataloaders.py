...
class ChallengeWeldingDataset(Dataset):
    ...
    def __init__(
        self, meta_df, resize=None, cache_strategy="remote", cache_dir=".cache"
    ):
        """
        This class defines a pytorch dataset that is connected to the challenge repository
        ...
        self.user_ui = challenge_welding.user_interface.ChallengeUI(
            cache_strategy, cache_dir
        )
        self.resize = resize
...
def create_pytorch_dataloader(
    ...
    if cache_strategy == "local":
        ...
        unique_id = int(input_df.iloc[0]["sample_id"].split("_")[-1]) + int(
            input_df.iloc[-1]["sample_id"].split("_")[-1]
        )  # unique _id associated with input dataframe
        print("cache_metadata_unique_id", unique_id)
        local_meta_path = (
            cache_dir + os.sep + "local_" + str(unique_id) + "_storage_meta.parquet"
        )
        ...
        if os.path.exists(local_meta_path):
            ...
            for sp in tqdm(local_meta_df.index):  # For each sample in dataset
                ...
                output_path = (
                    cache_dir + os.sep + target_image_name.replace("/", os.sep)
                )
                ...
                urllib.request.urlretrieve(
                    local_meta_df.loc[sp, "external_path"], output_path
                )
    ...
    challenge_dataset = ChallengeWeldingDataset(
        input_df, (540, 540), cache_strategy, cache_dir
    )
